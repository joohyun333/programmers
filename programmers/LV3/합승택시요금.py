...
def solution(n: int, s: int, a: int, b: int, fares: List[int]) -> int:
    graph: DefaultDict[int, int, int] = collections.defaultdict(list)
    for start, dis, fee in fares:
        graph[start].append((dis, fee))
        graph[dis].append((start, fee))

    def Dijkstra(s: int, d: int) -> DefaultDict[int, int]:
        queue: List[int, int] = [(0, s)]
        discoverd: List[bool] = [False] * (n + 1)
        dis: DefaultDict[int, int] = collections.defaultdict(list)
        while queue:
